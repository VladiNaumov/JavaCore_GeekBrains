...
public class ClientHandler__ {
    ...
    public ClientHandler__(Server server, Socket socket) throws IOException {
        this.server = server;
        this.socket = socket;
        this.in = new DataInputStream(socket.getInputStream());
        this.out = new DataOutputStream(socket.getOutputStream());

        /*создание отдельного потока подключения к серверу (Authentication server)*/
        new Thread(() -> {
            try {
                // Цикл авторизации
                while (true) {
